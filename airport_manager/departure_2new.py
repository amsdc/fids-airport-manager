...
class DataFrame(
    ...
    def __init__(self, parent, con, data, *args, **kwargs):
        ...
        try:
            ...
            cur.execute(
                "SELECT `etd`, `ofid`, `to`, `gate`, `status` "
                        "FROM `flight` WHERE "
            ...
            self.__heading_2 = tk.Label(self, text="ETD", **themes.get_label_attr(themes.get_style("header")))
            ...
            self.__heading_5 = tk.Label(self, text="Gate", **themes.get_label_attr(themes.get_style("header")))
            self.__heading_5.grid(row=0, column=4, **themes.get_grid_attr(themes.get_style("header")))
            self.__heading_6 = tk.Label(self, text="Status", **themes.get_label_attr(themes.get_style("header")))
            ...
            for data in cur.fetchall():
                ...
                l13 = tk.Label(self, text=self._data.get(data[2], data[2]), **themes.get_label_attr(style))
                l13.grid(row=i, column=3, **themes.get_grid_attr(style))

                l11 = tk.Label(self, text=data[].strftime("%H:%M"), **themes.get_label_attr(style))
                l11.grid(row=i, column=1, **themes.get_grid_attr(style))
                l12 = tk.Label(self, text=data[1], **themes.get_label_attr(style))
                l12.grid(row=i, column=2, **themes.get_grid_attr(style))
                l14 = tk.Label(self, text=data[3], **themes.get_label_attr(style))
                l14.grid(row=i, column=4, **themes.get_grid_attr(style))
                l15 = tk.Label(self, text=data[4], **themes.get_label_attr(style))
                l15.grid(row=i, column=5, **themes.get_grid_attr(style))
